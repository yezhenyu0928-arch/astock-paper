...
class S15CoreAllocation(BaseStrategy):
    ...
    def generate_orders(self, date, ctx, account):
        ...
        div_params = {
            ...
            "hold_n": 5, "max_per_industry": 2, "low_vol_pct": 0.35,
            "regime_downsize": True,
            "weights": {"dividend": 0.45, "low_vol": 0.25, "roe": 0.15, "valuation": 0.10, "news": 0.05},
        }
        # 质量成长袖: 放宽股息门槛, 重 ROE质量 + 估值(纳入高质量票)
        grw_params = {
            "min_dividend_yield": 0.015, "dividend_years": 2, "roe_years": 3, "roe_min": 0.10,
            "hold_n": 5, "max_per_industry": 2, "low_vol_pct": 0.35,
            "regime_downsize": True,
            "weights": {"dividend": 0.10, "low_vol": 0.15, "roe": 0.40, "valuation": 0.25, "news": 0.10},
        }
