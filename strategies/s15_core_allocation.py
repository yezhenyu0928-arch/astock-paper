...
class S15CoreAllocation(BaseStrategy):
    ...
    def generate_orders(self, date, ctx, account):
        ...
        min_dy = self.params.get("min_dividend_yield", 0.02)
        div_years = int(self.params.get("dividend_years", 2))
        roe_min = self.params.get("roe_min", 0.08)
        roe_years = int(self.params.get("roe_years", 2))
        g_min = self.params.get("growth_min", 0.0)
        g_max = self.params.get("growth_max", 1.0)
