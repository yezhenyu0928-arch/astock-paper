...
# ---------------------------------------------------------------------------
# s26 微盘规模因子(2026-07-27 冲"年化≥10%且夏普>1"目标)
#   主轴换成规模因子(cap 0.50): A股 2022-2024 微盘/小市值风格显著占优(公开研报共识),
#   持仓 10 只分散压波动(顶夏普), 动量只做辅助(0.20)。风控走 _RISK_RELAX['s26']。
# ---------------------------------------------------------------------------
class RefMicrocap(RefAggressiveSmallcap):
    """s26 微盘规模因子: cap 0.50 主导 + 10只分散 + 冻结豁免。逻辑复用 mf_core。"""
    pass


# ---------------------------------------------------------------------------
# s27 红利低波(防守进攻线): 高股息 0.40 + 低波 0.25 主导, 大盘段, 10 只分散。
#   2022-2025 A股最抗跌且有绝对收益的风格; 靠低波动把夏普顶上 1。
#   风控走 _RISK_RELAX['s27'](熔断15%/冻结豁免/止损15%)。
# ---------------------------------------------------------------------------
class RefDividendLowVol(RefAggressiveSmallcap):
    """s27 红利低波: dividend 0.40 + low_vol 0.25 + 大盘段 + 10只分散。逻辑复用 mf_core。"""
    pass


# ---------------------------------------------------------------------------
# s28 微盘涡轮版(2026-07-27 用户梯度目标第二档: 年化>30% / 回撤<30%)
#   在 s26@v7 最优骨架(规模+深度反转)上加杠杆: 更集中(4-5只)+熔断30%+止损35%。
#   风控走 _RISK_RELAX['s28']。诚实声明: 回撤可能接近30%,高波动换高收益。
# ---------------------------------------------------------------------------
class RefMicrocapTurbo(RefAggressiveSmallcap):
    """s28 微盘涡轮: 规模+反转因子, 高集中度, 风控第二档(熔断30%)。逻辑复用 mf_core。"""
    pass


# ---------------------------------------------------------------------------
# s29 小盘精选(复现国信金工《聚焦小盘股》2026-07-27):
#   全市场剔除ST/次新 → 总市值最小1/3 → 复合因子(规模+估值+反转)前N只等权,月频。
#   原文年化39.2%(中证2000超额28.7%)。本地复现依赖主板全量数据(backfill 补全后)。
#   风控走 _RISK_RELAX['s29'](待注册)。
# ---------------------------------------------------------------------------
class RefSmallcapSelect(RefAggressiveSmallcap):
    """s29 小盘精选: 市值最小1/3池 + 复合因子。逻辑复用 mf_core。"""
    pass


# ---------------------------------------------------------------------------
# s30 稳健精选(复现国信金工《稳健型选股策略探析》2026-07-27):
#   低估值+低波动+高股息合成稳健因子建池 → 动量增强精选,月频。
#   原文年化26.2%、回撤明显低于中证红利。与 s27 的区别: 加动量增强层、估值权重更高。
# ---------------------------------------------------------------------------
