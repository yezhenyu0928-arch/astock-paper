...
def _tencent_raw(code, start, end):
    ...
    if df is None or getattr(df, "empty", True):
        return None, None
    # 兼容 akshare 新旧两种返回结构:
    #  旧版: date/open/close/high/low/amount(实为成交量,手), 无 volume 列
    #  新版: date/open/close/high/low/volume(手)/turnover/amount(元)
    df = df.rename(columns={"date": "trade_date"})
    ...
    if has_vol and "amount" in df.columns:
        df["amount"] = pd.to_numeric(df["amount"], errors="coerce").fillna(0.0)  # 新版真实成交额(元)
    else:
        df["amount"] = df["volume"] * df["close"]              # 旧版无成交额,量×价估算
    df = df[(df["trade_date"] >= start) & (df["trade_date"] <= end)]
    return df[["trade_date", "open", "high", "low", "close", "volume", "amount"]].copy(), None
