...
def macro_exposure_mult(date, ctx, cfg=None):
    """大盘趋势择时总开关 → 仓位敞口系数(0.0~1.0)。

    核心判断: 沪深300(sh510300) 站上 MA200 → 满仓(1.0), 跌破 MA200 → 空仓(0.2以下)。
    MA200 是 A股公认的牛熊分界线, 站上才是牛市、才值得满仓做多。

    覆盖阈值由环境变量:
      MACRO_BASELINE   站上MA200后的常态仓位 (默认 1.0)
      MACRO_BEAR_FLOOR 跌破MA200后的极限仓位 (默认 0.20)
      MACRO_WEAK_MULT  跌破MA50后的仓位      (默认 0.50)
      MACRO_MA200_MULT 仅破MA200但未破MA50  (默认 0.30)
    无数据/异常 → 1.0(不干预)。"""
    import os
    try:
    ...
    base = float(os.environ.get("MACRO_BASELINE", "1.0"))
    bear_floor = float(os.environ.get("MACRO_BEAR_FLOOR", "0.25"))
    weak_mult = float(os.environ.get("MACRO_WEAK_MULT", "0.50"))
    ma200_mult = float(os.environ.get("MACRO_MA200_MULT", "0.35"))

    above50 = rg.get("aboveMa50")
    above200 = rg.get("aboveMa200")
    risk_ratio = rg.get("risk_ratio")

    # ── 牛熊总开关: MA200 是 A股牛熊分界线 ──
    # 跌破 MA200 = 熊市 → 极低仓位(0~0.30)
    # 站上 MA200 = 牛市/震荡 → 高仓位(0.50~1.0)
    if above200 is False:
        if above50 is False:
            trend_mult = bear_floor         # 跌破MA200+MA50 → 彻底防御(0.20)
        else:
            trend_mult = ma200_mult         # 仅破MA200但MA50尚在 → 偏空(0.30)
    elif above50 is False:
        trend_mult = weak_mult              # 站上MA200但跌破MA50 → 半仓(0.50)
    else:
        trend_mult = base                   # 站上MA200+MA50 → 满仓(1.0)

    # 市场风险比(收盘<MA200 成分占比)过高 → 再压一档
    if risk_ratio is not None and risk_ratio >= 0.5 and trend_mult > 0.30:
        trend_mult = 0.30

    return float(max(0.0, min(1.0, trend_mult)))
