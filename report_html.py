...
STRAT_META = {
    ...
    "s15_core_allocation@v2": {
        ...
        "rebalance": "每月最后交易日 · 等权10只 · 池=沪深300成分",
        "data_source": "行情/估值/分红/ROE:baostock(daily_bar/stock_annual/dividend); 成分股:index_members(沪深300); 宏观regime:macro_indicator; 新闻:news_raw/news_signal(历史回测区间待回灌,当前权重恒0)"},
    # ===== 2026-08-03 race_v2 新阵容(依据因子诊断重建, 混合中性化方案) =====
    "r1_small_rev@v1": {
        "name": "小市值反转", "risk": "★★★★☆ 中高", "fit": "≥10万",
        "tagline": "A股最强Alpha(1月反转)+小市值+低换手:反转(0.50)+规模(0.25)+低换手(0.15)+低波(0.10), 低波预筛60%+动量门槛, 月频10只, 主板小盘段。诊断ICIR反转-0.39/低波0.39。",
        "factors": [("1月反转(rev1m)", "50%"), ("小市值规模(size)", "25%"), ("低换手(amount)", "15%"),
                    ("低波(vol)", "10%"), ("防御管线", "低波预筛60% + 12-1动量门槛-10%"),
                    ("持仓控制", "月频10只·单行业≤3·轻量再平衡"),
                    ("风险过滤", "波动率自适应止损12% + 熔断25%")],
        "rebalance": "每月最后交易日(月频)调仓 · 持10只 · 池=主板小盘段(市值≥50亿)"},
    "r2_quality_value@v1": {
        "name": "质量价值", "risk": "★★★☆☆ 中", "fit": "≥10万",
        "tagline": "高质量(ROE)+价值(EP/PB)+低波+小盘倾斜(沪深300内小市值):ROE(0.30)+EP(0.20)+PB(0.15)+低波(0.20)+小盘(0.15)。沪深300池, 行业中性化, 月频10只。回测+9.3%/Calmar0.58(风险调整最优)。",
        "factors": [("ROE质量(roe)", "30%"), ("盈利市值比(ep)", "20%"), ("PB价值(bm)", "15%"),
                    ("低波(vol)", "20%"), ("小盘倾斜(size)", "15%"),
                    ("中性化", "行业中性化(真Alpha) + 低波预筛50% + 动量门槛"),
                    ("持仓控制", "月频10只·单行业≤3·沪深300成分"),
                    ("风险过滤", "波动率自适应止损10% + 熔断15%")],
        "rebalance": "每月最后交易日(月频)调仓 · 持10只 · 池=沪深300成分"},
    "r3_value_rev@v1": {
        "name": "深度价值反转", "risk": "★★★☆☆ 中", "fit": "≥10万",
        "tagline": "深度价值(PB)+1月反转+盈利市值比+低波, 中盘段, 行业中性化:PB(0.45)+反转(0.30)+EP(0.15)+低波(0.10)。主板中盘段, 月频10只。",
        "factors": [("PB价值(bm)", "45%"), ("1月反转(rev1m)", "30%"), ("盈利市值比(ep)", "15%"),
                    ("低波(vol)", "10%"), ("中性化", "行业中性化 + 低波预筛50% + 动量门槛"),
                    ("持仓控制", "月频10只·单行业≤3·主板中盘段"),
                    ("风险过滤", "波动率自适应止损12% + 熔断20%")],
        "rebalance": "每月最后交易日(月频)调仓 · 持10只 · 池=主板中盘段"},
    "r4_all_a_growth@v1": {
        "name": "全A价值低波", "risk": "★★★★☆ 中高", "fit": "≥10万",
        "tagline": "全A股(含科创/创业/北交)价值低波防御:PB(0.35)+低波(0.30)+反转(0.20)+EP(0.15)。100亿门槛缩池, 月频10只。覆盖主板策略不触及的板块。",
        "factors": [("PB价值(bm)", "35%"), ("低波(vol)", "30%"), ("1月反转(rev1m)", "20%"),
                    ("盈利市值比(ep)", "15%"), ("中性化", "行业中性化 + 低波预筛50% + 动量门槛"),
                    ("持仓控制", "月频10只·单行业≤3·全A池(市值≥100亿)"),
                    ("风险过滤", "波动率自适应止损15% + 熔断40%")],
        "rebalance": "每月最后交易日(月频)调仓 · 持10只 · 池=全A(含科创/创业/北交)"},
    "r5_etf_momentum@v1": {
        "name": "ETF全球防御", "risk": "★★☆☆☆ 中低", "fit": "≥3万",
        "tagline": "宽基+红利+纳指+国债 ETF 防御配置:低波(0.60)+动量(0.40), 月频4只。跨资产分散(沪深300/红利低波/纳指/国债), 回撤仅9.6%。",
        "factors": [("低波(vol)", "60%"), ("动量(rev1m)", "40%"),
                    ("ETF池", "沪深300/红利低波/纳指/国债 4只"),
                    ("持仓控制", "月频4只·等权"),
                    ("风险过滤", "止损8% + 熔断20%")],
        "rebalance": "每月调仓 · 持4只 · 池=宽基/红利/纳指/国债ETF"},
    "r6_lowvol_def@v1": {
        "name": "低波防御", "risk": "★★☆☆☆ 中低", "fit": "≥10万",
        "tagline": "主板低波+质量+价值防御:低波(0.40)+ROE(0.25)+PB(0.20)+EP(0.15)。大盘段, 行业中性化, 周频10只。去掉MA60择时闸门(弱市空仓错过反弹)。",
        "factors": [("低波(vol)", "40%"), ("ROE质量(roe)", "25%"), ("PB价值(bm)", "20%"),
                    ("盈利市值比(ep)", "15%"), ("中性化", "行业中性化 + 低波预筛50% + 动量门槛"),
                    ("持仓控制", "周频10只·单行业≤3·主板大盘段"),
                    ("风险过滤", "波动率自适应止损8% + 熔断15%")],
        "rebalance": "每周最后交易日(周频)调仓 · 持10只 · 池=主板大盘段"},
}
